...
腾讯AI语音合成 API 在网站上面无法查询到相关收费项目，在开放平台同样无法搜索相关收费信息


3.1 比较分析

|比较分析|科大讯飞 语音合成|百度云 语音合成|腾讯AI 语音合成|网易AI 语音合成|
|-------|---------------|--------------|-------------|-------------|
|优先级  |:one:          |:two:         |:three:      |未开放       |
|成熟度  | :star: :star: :star: | :star: :star: |:star: |未开放       |
|性价比  |认证领取免费额度-高|认证领取免费额度-低|  无相关文档信息     |未开放|
|准确度  | :star: :star: :star:  |:star: :star: |:star:      |未开放       |
|拓展性  | :star: :star: :star:  |:star: :star:  |:star:    |未开放      |
|优势    |可针对不同用户进行比较，如新用户/老客户/回头客，88+种语音调用可划分等级，特色发声人78人包括场景与声音年龄段筛选个性化，精品发声人5人，基础发声人5人免费，语速+音量，配备背景音乐符合词的个性，声音比腾讯AI好听，在换行之间不会讲得很快，有间隔来讲不像百度无间隔稍作停顿|4种选择声音，语速+音调+音量，代码开发文档还比较友好，甚至直接的调用代码都给到 GitHub 上直接修改参数调用就好，还是比较良心的| 腾讯AI Lab 和 腾讯优图，4种声音类型，音高+音量+音色+语速，在段落之间也就是换行间比百度要慢速一点，听得更加舒服，1男3女声，可利用情感来进行分析|未开放      |
|劣势    |250词限制，找不到文档就是按照上课流程来进行代码的调用|200字限制，很着急直接念完没有停顿，2男声2女声|50字限制，在代码层次上语音合成只有PHP的代码没有pythn代码，官方压根没有合成示例文档，全自己摸索，SDK 都开发出来了，示例代码给一下能死，怪不得没人用。在合成效果上讲，声音难听爆了，语音文件还得解码再 I/O, 吃饱了撑的，怪不得没人用，百度语音合成效果比你强多了|未开放网页端，只有微信小程序体验版 网易 AI 体验中心，限制50字，语速只有正常慢速快速之分，没有API可以调用，没有文档|
|最适用  | :heavy_check_mark: 开发使用多样性选科大 | :heavy_check_mark: 代码调用方便简洁性选百度云    | :o:   | :x:  |
...
|使用风险报告|Face++ 人脸融合|百度AI 人脸融合|腾讯AI 人脸融合|
|-----------|--------------|--------------|--------------|
|API 类别属性|人像处理      |人像特效       |未开放         |
|市场竞争程度|技术较为成熟市场占有率大|技术成熟度次之市场占有率一般|无法考证|
|输入输出限制|输入多场景变换输出高拟合人像但原图像可识别度高|输入单一场景输出低拟合人像但原图像可识别度低|无法考证|
|定价高低程度|按量:0.1元/次-性价比低|前500次免费，超出 0.04元/次-性价比高|无法考证|
|可替代程序库|不可改用自己开发的代码及数据库而不用 API|可改用自己开发的代码及数据库而不用 API|无法考证|
|调用难易程度|开发文档详细，调用权限较少，不需获取 access_token ，代码实现有 python 等轻量级语言，但不够明确和完善，必须要开发者来直接进行提取不能直接上手，开发效率没有符合预期|代码开发实现效率低下，文档不全，对新手不友好，但对有基础会比较简单|无法考证|
|调用响应程度|:star: :star: |:star: |无法考证|
|文档完善程度|:star: :star:|:star: |无法考证|
|用户体验程度|:star: :star:|:star:|无法考证|
|调用适用程度| :heavy_check_mark:    | :o:            | :x:  |
